...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case Nil => Nil
    case node :: Nil => trees
    case left :: right :: tail =>  (makeCodeTree(left, right) :: tail).sortBy(tree => weight(tree))
  }
   ...
   */
  def until(termination: List[CodeTree] => Boolean, reduce: List[CodeTree] => List[CodeTree])(input: List[CodeTree]): List[CodeTree] = {
      if (termination(input)) input else until(termination, reduce)(reduce(input))
  }
    ...
    def traverse(node: CodeTree, bits: List[Bit]) : List[Char] = {
      bits match {
        ...
        case bit :: tail => node match {
            case Leaf(x, _) => x :: decode(tree, bits)
            case Fork(left, right, _, _) => if (bit == 0) traverse(left, tail) else traverse(right, tail)
          }
      }
    }
    traverse(tree, bits)
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    text.flatMap(x => encodeChar(tree)(x))
  }
